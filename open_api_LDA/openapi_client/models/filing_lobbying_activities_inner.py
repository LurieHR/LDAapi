...
class FilingLobbyingActivitiesInner(BaseModel):
    """
    FilingLobbyingActivitiesInner
    """ # noqa: E501
    general_issue_code: Optional[StrictStr] = None
    general_issue_code_display: Optional[StrictStr] = None
    description: Optional[StrictStr] = None
    foreign_entity_issues: Optional[StrictStr] = None
    lobbyists: Optional[List[FilingLobbyingActivitiesInnerLobbyistsInner]] = None
    government_entities: Optional[List[FilingLobbyingActivitiesInnerGovernmentEntitiesInner]] = None
    __properties: ClassVar[List[str]] = ["general_issue_code", "general_issue_code_display", "description", "foreign_entity_issues", "lobbyists", "government_entities"]

    @field_validator('general_issue_code')
    def general_issue_code_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ACC', 'ADV', 'AER', 'AGR', 'ALC', 'ANI', 'APP', 'ART', 'AUT', 'AVI', 'BAN', 'BNK', 'BEV', 'BUD', 'CIV', 'CHM', 'CAW', 'CDT', 'COM', 'CPI', 'CON', 'CSP', 'CPT', 'DEF', 'DIS', 'DOC', 'ECN', 'EDU', 'ENG', 'ENV', 'FAM', 'FIN', 'FIR', 'FOO', 'FOR', 'FUE', 'GAM', 'GOV', 'HCR', 'HOM', 'HOU', 'IMM', 'IND', 'INS', 'INT', 'LBR', 'LAW', 'MAN', 'MAR', 'MIA', 'MED', 'MMM', 'MON', 'NAT', 'PHA', 'POS', 'RRR', 'RES', 'REL', 'RET', 'ROD', 'SCI', 'SMB', 'SPO', 'TAR', 'TAX', 'TEC', 'TOB', 'TOR', 'TRD', 'TRA', 'TOU', 'TRU', 'URB', 'UNM', 'UTI', 'VET', 'WAS', 'WEL']):
            raise ValueError("must be one of enum values ('ACC', 'ADV', 'AER', 'AGR', 'ALC', 'ANI', 'APP', 'ART', 'AUT', 'AVI', 'BAN', 'BNK', 'BEV', 'BUD', 'CIV', 'CHM', 'CAW', 'CDT', 'COM', 'CPI', 'CON', 'CSP', 'CPT', 'DEF', 'DIS', 'DOC', 'ECN', 'EDU', 'ENG', 'ENV', 'FAM', 'FIN', 'FIR', 'FOO', 'FOR', 'FUE', 'GAM', 'GOV', 'HCR', 'HOM', 'HOU', 'IMM', 'IND', 'INS', 'INT', 'LBR', 'LAW', 'MAN', 'MAR', 'MIA', 'MED', 'MMM', 'MON', 'NAT', 'PHA', 'POS', 'RRR', 'RES', 'REL', 'RET', 'ROD', 'SCI', 'SMB', 'SPO', 'TAR', 'TAX', 'TEC', 'TOB', 'TOR', 'TRD', 'TRA', 'TOU', 'TRU', 'URB', 'UNM', 'UTI', 'VET', 'WAS', 'WEL')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in lobbyists (list)
        _items = []
        if self.lobbyists:
            for _item_lobbyists in self.lobbyists:
                if _item_lobbyists:
                    _items.append(_item_lobbyists.to_dict())
            _dict['lobbyists'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in government_entities (list)
        _items = []
        if self.government_entities:
            for _item_government_entities in self.government_entities:
                if _item_government_entities:
                    _items.append(_item_government_entities.to_dict())
            _dict['government_entities'] = _items
        # set to None if description (nullable) is None
        # and model_fields_set contains the field
        if self.description is None and "description" in self.model_fields_set:
            _dict['description'] = None

        # set to None if foreign_entity_issues (nullable) is None
        # and model_fields_set contains the field
        if self.foreign_entity_issues is None and "foreign_entity_issues" in self.model_fields_set:
            _dict['foreign_entity_issues'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "general_issue_code": obj.get("general_issue_code"),
            "general_issue_code_display": obj.get("general_issue_code_display"),
            "description": obj.get("description"),
            "foreign_entity_issues": obj.get("foreign_entity_issues"),
            "lobbyists": [FilingLobbyingActivitiesInnerLobbyistsInner.from_dict(_item) for _item in obj["lobbyists"]] if obj.get("lobbyists") is not None else None,
            "government_entities": [FilingLobbyingActivitiesInnerGovernmentEntitiesInner.from_dict(_item) for _item in obj["government_entities"]] if obj.get("government_entities") is not None else None
        })
        return _obj
