...
class FilingAffiliatedOrganizationsInner(BaseModel):
    """
    FilingAffiliatedOrganizationsInner
    """ # noqa: E501
    name: Optional[StrictStr] = None
    url: Optional[Annotated[str, Field(strict=True)]] = None
    address_1: Optional[StrictStr] = None
    address_2: Optional[StrictStr] = None
    address_3: Optional[StrictStr] = None
    address_4: Optional[StrictStr] = None
    ...
    ppb_city: Optional[StrictStr] = None
    ppb_state: Optional[StrictStr] = None
    ppb_state_display: Optional[StrictStr] = None
    ppb_country: Optional[StrictStr] = None
    ppb_country_display: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["name", "url", "address_1", "address_2", "address_3", "address_4", "city", "state", "state_display", "zip", "country", "country_display", "ppb_city", "ppb_state", "ppb_state_display", "ppb_country", "ppb_country_display"]
    ...
    def url_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(?:[a-z0-9.+-]*):\/\/(?:[^\s:@\/]+(?::[^\s:@\/]*)?@)?(?:(?:0|25[0-5]|2[0-4]\d|1\d?\d?|[1-9]\d?)(?:\.(?:0|25[0-5]|2[0-4]\d|1\d?\d?|[1-9]\d?)){3}|\[[0-9a-f:.]+\]|([a-z¡-￿0-9](?:[a-z¡-￿0-9-]{0,61}[a-z¡-￿0-9])?(?:\.(?!-)[a-z¡-￿0-9-]{1,63}(?<!-))*\.(?!-)(?:[a-z¡-￿-]{2,63}|xn--[a-z0-9]{1,59})(?<!-)\.?|localhost))(?::\d{2,5})?(?:[\/?#][^\s]*)?\z", value):
            raise ValueError(r"must validate the regular expression /^(?:[a-z0-9.+-]*):\/\/(?:[^\s:@\/]+(?::[^\s:@\/]*)?@)?(?:(?:0|25[0-5]|2[0-4]\d|1\d?\d?|[1-9]\d?)(?:\.(?:0|25[0-5]|2[0-4]\d|1\d?\d?|[1-9]\d?)){3}|\[[0-9a-f:.]+\]|([a-z¡-￿0-9](?:[a-z¡-￿0-9-]{0,61}[a-z¡-￿0-9])?(?:\.(?!-)[a-z¡-￿0-9-]{1,63}(?<!-))*\.(?!-)(?:[a-z¡-￿-]{2,63}|xn--[a-z0-9]{1,59})(?<!-)\.?|localhost))(?::\d{2,5})?(?:[\/?#][^\s]*)?\z/")
        return value
    ...
    def state_validate_enum(cls, value):
        ...
        if value not in set(['AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'DC', 'FL', 'GA', 'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY', 'AB', 'BC', 'MB', 'NB', 'NL', 'NT', 'NS', 'NU', 'ON', 'PE', 'QC', 'SK', 'YT']):
            raise ValueError("must be one of enum values ('AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'DC', 'FL', 'GA', 'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY', 'AB', 'BC', 'MB', 'NB', 'NL', 'NT', 'NS', 'NU', 'ON', 'PE', 'QC', 'SK', 'YT')")
        return value
    ...
    @field_validator('ppb_state')
    def ppb_state_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    @field_validator('ppb_country')
    def ppb_country_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # set to None if address_3 (nullable) is None
        # and model_fields_set contains the field
        if self.address_3 is None and "address_3" in self.model_fields_set:
            _dict['address_3'] = None

        # set to None if address_4 (nullable) is None
        # and model_fields_set contains the field
        if self.address_4 is None and "address_4" in self.model_fields_set:
            _dict['address_4'] = None
        ...
        # set to None if ppb_city (nullable) is None
        # and model_fields_set contains the field
        if self.ppb_city is None and "ppb_city" in self.model_fields_set:
            _dict['ppb_city'] = None

        # set to None if ppb_state (nullable) is None
        # and model_fields_set contains the field
        if self.ppb_state is None and "ppb_state" in self.model_fields_set:
            _dict['ppb_state'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ppb_city": obj.get("ppb_city"),
            "ppb_state": obj.get("ppb_state"),
            "ppb_state_display": obj.get("ppb_state_display"),
            "ppb_country": obj.get("ppb_country"),
            "ppb_country_display": obj.get("ppb_country_display")
        })
        return _obj
